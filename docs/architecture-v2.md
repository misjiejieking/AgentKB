...
│  │  ┌────────────┬──────────────┬──────────────────┐  │    │
│  │  │Working Mem │ LongTerm Mem │ Knowledge Graph  │  │    │
│  │  │(会话上下文) │(向量化持久)   │(实体关系抽取)    │  │    │
│  │  └────────────┴──────────────┴──────────────────┘  │    │
│  └────────────────────────────────────────────────────┘    │
│        │                                                    │
...
│   sessions │ messages │ knowledge_chunks │ traces │ metrics │
└─────────────────────────────────────────────────────────────┘
```
...
**导出器**：Console(开发) / File(生产) / LangFuse / LangSmith / Prometheus metrics
...
Supervisor Agent (主控)
    ...
    └── SocialWriter        — 社媒内容 (平台适配→风格→优化)
            │
            └── 共享 Memory (Working + LongTerm + Reflection)
```
...
├── memory/                 # 【新增】增强记忆层
│   ├── __init__.py
│   ├── working.py          # 工作记忆 (会话上下文窗口)
│   ├── long_term.py        # 长期记忆 (向量化 + 重要性评分)
│   └── knowledge_graph.py  # 知识图谱 (实体关系抽取)
│
...
memory:
  ...
  knowledge_graph_enabled: false      # 知识图谱 (P2)
